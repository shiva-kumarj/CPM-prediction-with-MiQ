...
def train_test_split(target_date, df):
...
if __name__ == '__main__':
    ...
    print("Path to store the model")
    model_save_path = filedialog.asksaveasfilename()

    # Read dataframe
    train, test, target_date = read_data(test_path, train_path)
    ...
    df_train, y_train, df_test = train_test_split(target_date, final_df)
    ...
    # Write model to disc
    #model_save_path = 'r'code\output\catboostmodel_1.pkl'
    #pickle.dump(cbr_model, open(r'code\output\catboostmodel_1.pkl', 'wb'))
    pickle.dump(cbr_model, open(model_save_path, 'wb'))

    print("Trained model at : {}".format(model_save_path))
